...
#ifndef _ADAFRUIT_ST7735AH_
#define _ADAFRUIT_ST7735AH_
...
public:
	...
	// Drawing a bitmap from file on SD card
	int loadSDBitmap(const char *);
	void setBitmapWindow(uint8_t = 0, uint8_t = 0);
	void drawSDBitmap();
	template <class SZ>
	SZ readBytes();
	// ======================================
...
SZ Adafruit_ST7735::readBytes() {
	SZ result;
	for (int b = 0; b<(int) sizeof(SZ); b++) // result[0] is LSB
		((uint8_t *)&result)[b] = file.read();
	return result;
}
